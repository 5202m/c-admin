...
var room = {
    ...
    getVisitorList: function(nickname) {
        ...
        try {
            ...
            $.getJSON('/getVistiorByName', { groupType: room.userInfo.groupType, groupId: room.userInfo.groupId, nickname: nickname }, function(data) {
                if (data) {
                    $(".searchResult").show();
                    var onlineStatus = 0,
                        userId = '';
    ...
    setEvent: function() {
         ...
         * 搜索@记录
         */
        $('#searchAtLogBtn').click(function() {
            room.searchAtLog();
        });
        $('#searchAtLog').keydown(function(e) {
            if (e.keyCode == 13) {
                room.searchAtLog();
            }
        });
    ...
    initGagDate: function() {
        ...
        common.getJson('getUserGag', { data: JSON.stringify(data.set) }, function(result) {
            if (result) {
    ...
    setUserGag: function(data, isVisitor) {
        common.getJson('setUserGag', { data: JSON.stringify(data), isvisitor: isVisitor }, function(result) {
            if (!isVisitor) {
                $('.not-talk').addClass('dn');
            } else {
    ...
    teacherPointEdit: function(editDom, saveDom, cancelDom) {
        ...
        $(saveDom).click(function() {
            ...
            common.getJson('modifyArticle', { data: JSON.stringify(param), where: JSON.stringify(where) }, function(result) {
                if (result.isOK) {
    ...
    fillWhHistory: function() {
        ...
        if (!$.isArray(room.whHistoryUserObj.whUserList)) {
            ...
            common.getJson('getLastTwoDaysMsg', { data: JSON.stringify(params) }, function(result) {
                if (result) {
     ...
     * 搜索@记录
     */
    searchAtLog: function() {
        var atTxt = $('#searchAtLog').val();
        if (common.isValid(atTxt)) {
            $('#talk_top_id .ss-tk-info').hide();
            $('#talk_top_id .ss-tk-info').each(function() {
                var $this = $(this);
                if ($this.children('span').text().indexOf(atTxt) > -1) {
                    $this.show();
                }
            });
        } else {
            $('#talk_top_id .ss-tk-info').show();
        }
    },
    /**
     * 计算宽度
