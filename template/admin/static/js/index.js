...
var index = {
    ...
    login: function() {
        ...
        if (common.isBlank(userId)) {
            ...
            common.getJson('/login', { userId: userId, password: password }, function(data) {
                if (data.isOK) {
                    location.reload();
                } else {
    ...
    joinRoom: function() {
        $('.lst_p .rm_p .rm_list li').click(function() {
            ...
            $(this).addClass('on');
            var groupId = $(this).attr('r'),
                groupType = $(this).attr('t'),
                groupName = $(this).parent().prev().text(),
                roomName = $(this).children('span').text();
            var iframeSrc = "/room?groupId=" + groupId + "&groupType=" + groupType + "&roomName=" + groupName + "【" + roomName + "】";
            if (index.isNw) {
